...
export function useAutoMode() {
  ...
  const runAutoMode = async (basePrompt: string, modelConfig: ModelConfig) => {
    ...
    console.log("🚀 Auto Mode starting with:", { 
      basePrompt: basePrompt ? basePrompt.substring(0, 30) + "..." : "undefined", 
      modelConfig 
    });
    ...
    try {
      ...
      try {
        ...
        if (generatedPrompt.length < 100 || generatedPrompt === basePrompt) {
      ...
      try {
        console.log("🔄 Calling generateVariations API with meta prompt length:", generatedPrompt.length);
        variationContents = await generateVariations(generatedPrompt, modelConfig);
        
        // Validate variations
        if (variationContents.length < 3) {
          console.error("❌ Not enough variations generated:", variationContents.length);
